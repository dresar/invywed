        ...
        <div class="col-xl-12 col-lg-12 mb-4">
            <div class="card">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    ...
                    $durasi = '+'.$trial.' days';
                    $tglDaftar = $order[0]->created_at ?? null;
                    $tglExp = strtotime($durasi, strtotime($tglDaftar ?? 'now'));
                    $tglExpFormated = date("d-m-Y H:i",$tglExp);
                    $today = strtotime('now');
                    $tglBayar = $pembayaran[0]->tglBayar;
                    $aktif = '+'.$masa_aktif.' days';
                    $tglNonaktif= strtotime($aktif, strtotime($tglBayar ?? 'now'));
                    $tglNonaktifFormated = date("d-m-Y H:i A",$tglNonaktif);
                    $expiry = strtotime($pembayaran[0]->transaction_expired ?? 'now');
