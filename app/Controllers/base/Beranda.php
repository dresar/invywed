...
class Beranda extends Controller
{
	...
	protected $TemaModel;
	protected $VideoCategoriesModel;
	protected $TemaCategoriesModel;
	protected $uri;
	...
	public function __construct() {
		...
		$this->uri = $request->getUri(); //class request digunakan untuk request uri/url - menggunakan getUri() method
    }

	public function index()
	{
		// BLOCK: Jika URI mengandung 'admin', langsung throw 404
		// Cek dari multiple sources untuk memastikan
		$uri = $this->uri->getPath();
		$uriTrimmed = ltrim($uri, '/'); // Remove leading slash
		$segment1 = $this->uri->getSegment(1);
		$requestUri = $_SERVER['REQUEST_URI'] ?? '';
		
		// Cek apakah URI dimulai dengan 'admin' atau segment pertama adalah 'admin'
		// Cek dari semua sumber yang mungkin
		if ($segment1 === 'admin' 
			|| $uriTrimmed === 'admin' 
			|| strpos($uriTrimmed, 'admin') === 0 
			|| strpos($uriTrimmed, 'admin/') === 0
		...
		// #region agent log
		$logFile = (defined('FCPATH') ? FCPATH : realpath(__DIR__ . '/../../../')) . '.cursor' . DIRECTORY_SEPARATOR . 'debug.log';
		$logDir = dirname($logFile);
		if (!is_dir($logDir)) @mkdir($logDir, 0755, true);
		$logEntry = json_encode(['id'=>'log_'.time().'_beranda_index_start','timestamp'=>time()*1000,'location'=>'Beranda.php:32','message'=>'Beranda index called','data'=>[],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
		...
		try {
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_get_themes','timestamp'=>time()*1000,'location'=>'Beranda.php:40','message'=>'Getting themes','data'=>[],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
			...
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_themes_done','timestamp'=>time()*1000,'location'=>'Beranda.php:44','message'=>'Themes retrieved','data'=>['temaCount'=>is_countable($data['tema']) ? count($data['tema']) : 'N/A'],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
			...
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_get_setting','timestamp'=>time()*1000,'location'=>'Beranda.php:50','message'=>'Getting setting','data'=>[],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
			...
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_setting_done','timestamp'=>time()*1000,'location'=>'Beranda.php:54','message'=>'Setting retrieved','data'=>['settingCount'=>is_countable($data['setting']) ? count($data['setting']) : 'N/A'],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
			...
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_data_ready','timestamp'=>time()*1000,'location'=>'Beranda.php:62','message'=>'All data retrieved successfully','data'=>['hasTema'=>isset($data['tema']),'hasSetting'=>isset($data['setting']),'hasPaket'=>isset($data['paket'])],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
			
		} catch (\Exception $e) {
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_exception','timestamp'=>time()*1000,'location'=>'Beranda.php:66','message'=>'Exception in data retrieval','data'=>['message'=>$e->getMessage(),'file'=>$e->getFile(),'line'=>$e->getLine(),'trace'=>substr($e->getTraceAsString(),0,500)],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
		...
		// #region agent log
		$logEntry = json_encode(['id'=>'log_'.time().'_beranda_view_start','timestamp'=>time()*1000,'location'=>'Beranda.php:75','message'=>'Rendering view','data'=>['view'=>$data['view']],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
		@file_put_contents($logFile, $logEntry, FILE_APPEND);
		// #endregion
		
		try {
			$output = view('base/beranda/layout', $data);
			
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_view_success','timestamp'=>time()*1000,'location'=>'Beranda.php:81','message'=>'View rendered successfully','data'=>['outputLength'=>strlen($output)],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
			...
			// #region agent log
			$logEntry = json_encode(['id'=>'log_'.time().'_beranda_view_exception','timestamp'=>time()*1000,'location'=>'Beranda.php:85','message'=>'Exception in view rendering','data'=>['message'=>$e->getMessage(),'file'=>$e->getFile(),'line'=>$e->getLine(),'trace'=>substr($e->getTraceAsString(),0,500)],'sessionId'=>'debug-session','runId'=>'run1','hypothesisId'=>'A']) . "\n";
			@file_put_contents($logFile, $logEntry, FILE_APPEND);
			// #endregion
