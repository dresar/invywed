...
class Dashboard extends Controller
{
    ...
    public function do_auth(){
        try {
            ...
            log_message('info', "User login attempt - Email: $email, IP: $ipAddress");
            ...
            if(count($hasil) > 0)
            {
                ...
                // Log to CodeIgniter log
                log_message('info', "User login successful - ID: {$hasil[0]->id}, Username: {$hasil[0]->username}, Email: $email, IP: $ipAddress");
            ...
            else
            {
                ...
                // Log to CodeIgniter log
                log_message('warning', "User login failed - Email: $email, IP: $ipAddress - Invalid credentials");
            ...
            log_error('Error in do_auth', $e);
            log_message('error', "User login error - " . $e->getMessage() . " - Email: " . ($this->request->getPost('email') ?? 'unknown'));
            $this->session->setFlashdata('errors', ['Terjadi kesalahan sistem']);
            return redirect()->to(base_url('/login'));
        }
    }
    ...
    // Method login sudah dipindah ke Auth controller
...
Balas *OK* agar bisa diklik Link Undangan';
    ...
    public function prosesExcel()
	{
		try {
			...
			log_message('error', 'Error in prosesExcel: ' . $e->getMessage());
			session()->setFlashdata("errors", ["Error: " . $e->getMessage()]);
			return redirect()->to('/user/tamu');
		}
	}
