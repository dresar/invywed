...
class Order extends Controller
{
	...
	public function __construct() {
		try {
			...
			log_error_debug('Order.php:11', 'Order constructor started');
			...
			log_error_debug('Order.php:22', 'Order constructor completed');
		} catch (\Exception $e) {
			log_error('Error in Order constructor', $e);
			throw $e;
		}
    }
	...
	public function index()
	{
		try {
			...
			log_error_debug('Order.php:22', 'Order index started', ['segment' => $this->uri->getSegment(3)]);
		...
		if($check == 1 || $kode == '1'){
			log_error_debug('Order.php:70', 'Order index returning view', ['checkpoint' => $check]);
			return view('base/order/order_layout',$data);
		}else{
			log_error_debug('Order.php:73', 'Order index redirecting', []);
			return redirect()->route('order/any');
		}
		} catch (\Exception $e) {
			log_error('Error in Order index', $e);
			return redirect()->to(base_url('/tema'));
		}
	}
